...
par(mar=c(6,5,4,2))
if(analysis=="baypass")pdf(paste0("figures/",candidate_genes[i],"_baypass.pdf"))
...
if(analysis=="bglmer")pdf(paste0("figures/",candidate_genes[i],"_bglmer.pdf"))
...
pdf(file = paste0("candidate_gene_baypass",list_gene[gene],".pdf"), width=10, height=15)
par(mfrow=c(5,2))
for (gene in c(1:5)){

gene_for_plot<- list_gene[gene]

results_bf <- data.frame(fread(paste0("Baypass_analysis/all_chr_baypass.csv"), sep = ","))
